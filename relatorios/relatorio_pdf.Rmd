...
* \textcolor{blue}{A construção de empaços de busca considerou espaços discretizados na forma de matrizes quadradas do tipo $M_{(i,j)}$ com $i = \{8, 9, ..., 150\}$ e $j = \{8, 9, ..., 150\}$ dentro da área de 800 x 800. Cada (i,j) representa a possível localização de um ponto de acesso.}

* \textcolor{blue}{Para ilustrar a construção do espaço discretizado de busca foram consideradas as matrizes de $M_{8,8}$ até $M_{128,128}$.}


![](../imagens/espacos_discretos.png){width=70%, height=100%}
...
* \textcolor{blue}{O algoritimo completo considerou espaços discretos entre 8 x 8 até 150 x 150.}

* [\textcolor{orange}{LINK da Apresentação em Slides: Como Funciona o Algoritmo}](https://rpubs.com/Drope/algoritmo)
...
```{Rcpp eval = F}
double square(double x){double squared = x * x; return squared;}
...
for(intervalo in seq(8, 150, by = 1)){
  ...
  vencedores <- NULL; contador <- 0 ;Mbps <- NULL; viola <- NULL
  inicio <- Sys.time()
  while(contador < 475){
    cobertura <- NULL
    populoso <- 1
    ...
    if(megas < 150){
      ...
      contador <- contador + length(cobertura)} else { viola <- c(viola, populoso)}}
  fim <- Sys.time()
...
![](../imagens/convergencia.png){width=100%, height=100%}


* \textcolor{blue}{O Ponto na coordenada 8 do eixo $Espaço_{(x,x)}$ e 20 no eixo Nº de Pontos de Acesso, corresponde a varredura do espaço de busca $M_{(8,8)}$ que atinge a solução ótima de mínimo local com 20 Pontos de Acesso.}
...
![](../imagens/custo_computacional.png){width=100%, height=100%}

* \textcolor{blue}{O Ponto na coordenada 8 do eixo $Espaço_{(x,x)}$ e 0,2234 no eixo Tempo (segundos), corresponde a varredura do espaço de busca $M_{(8,8)}$ que atinge a solução ótima de mínimo local com custo computacional de 0.2 segundos.}
...
![](../imagens/solucao_80x80.png){width=100%, height=100%}
