...
* Na última reunião a __unidade de decisão__ se mostrou flexível em alterar algumas restrições como número de __Pd's__ cobertos e a capacidade máxima em __Mbps__ de cada __Pa__. A unidade de decisão enviou um item da ata dessa reunião com os dizeres.

* _A empresa precisa garantir a __qualidade do serviço__ tendo em vista os resultados referentes aos __Pa's__ que iriam operar em sua capacidade máxima e isso deve ser considerado uma das prioridades. Mas __reduzir custo__ deve ser a prioridade principal na escolha da solução. Contudo é fato conhecido no registro histórico de operações da empresa que a __diferença__ entre clientes __registrados__ e __presentes__ em conferências é geralmente maior que __5%__ e, portanto, o relaxamento desse critério também deve ser considerado prioridade. A terceira e última prioridade seria a variação de __ambos os parâmetros__. Solicitar ao departamento de __BI__ que gere __3__ diferentes cenários com base nessas diretrizes para orientar a unidade de decisão._  

* Como resposta o departamento de departamento __BI__ implementou o método __AHP__ _Analytic Hierarchy Process_ para auxiliar a __unidade de decisão__ na comparação. Para esta tarefa foram criados 3 robôs posicionadores que representam possíveis cenários a serem considerados pela __unidade de decisão__:
...
```{r fig.align='center', fig.width=12, fig.hight=14, echo = FALSE}
...
* O critério Máximo Mbps considerou a comparação dos 3 otimizadores em relação a variação no limite máximo da soma de Mbps fornecido por cada Pa. 


```{r fig.align='center', fig.width=10, fig.hight=12, echo = FALSE}
...
* O otimizador __2__ tem prioridade de  __Máximo Mbps__ em relação aos otimizadores __1__ e __3__ e o otimizador __3__ tem prioridade em relação ao otimizador __1__.
...
* A unidade de decisão gostou da solução e optou pelo otimizador __2__. Porém solicitou que o modelo do algoritmo otimizador fosse colocado em produção para __operações futuras__. A unidade de decisão também solicitou que fosse construída uma __interface gráfica__ para utilização do modelo com __botões__ para os parâmetros de __Mbps__, __Cobertura __e __Granularidade do Espaço de Busca__.

* Para atender a demanda da unidade de decisão o departamento de BI da empresa desenvolveu o aplicativo __Posicionador__.
